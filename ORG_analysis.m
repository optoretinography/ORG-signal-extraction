...
%   to a flash sequence of green-UV-green-UV.
...
%   between adjacent flashes was 100 ms.
%
...
file_name = 'I_reg';
FigSaveFolder = [RegisteredDataFolder 'figure/']; % folder to save exported figures
ResultSaveFolder = [RegisteredDataFolder 'analysis/']; % folder to save ORG signals
if ~(exist(FigSaveFolder, 'dir') && exist(ResultSaveFolder, 'dir'))
    mkdir(FigSaveFolder);
...
loc = zeros(N_x*(2*T_dZ+1), 2); % indices of individual pixels in the target layer

I_Tref = I_reg .* conj(repmat(I_reg(:,:,1), 1, 1, N_t)); % calculate time referenced OCT signals
...
save([ResultSaveFolder, 'ORG_signal.mat'], 'samp_time', 'ORG_OPL_trace');
